...
import { gotchiverseSinkEvents, gotchiverseSinks } from "../queries";

interface SinksRes {
  mintInstallationEvents: MintInstallationEvents[];
...
interface MintInstallationEvents {
  id: string;
...
const Home: NextPage = () => {
  const [instEvents, setInstEvents] = useState<MintInstallationEvents[]>();
  ...
  const fetchSinks = async () => {
    const lastHour = (Date.now() / 1000 - timeFrame).toFixed();
    const res: SinksRes = await request(
      gotchiverseUrl,
      gotchiverseSinkEvents(lastHour)
    );

    console.log("res:", res.mintInstallationEvents);
    ...
    res.mintInstallationEvents.forEach((type) => {
      let fudIns = Number(
        ethers.utils.formatEther(type.installationType.alchemicaCost[0])
      );
      let fomoIns = Number(
        ethers.utils.formatEther(type.installationType.alchemicaCost[1])
      );
      let alphaIns = Number(
        ethers.utils.formatEther(type.installationType.alchemicaCost[2])
      );
      let kekIns = Number(
        ethers.utils.formatEther(type.installationType.alchemicaCost[3])
      );
    ...
    setInstEvents(res.mintInstallationEvents);
      ...
      >
        <div>
          ...
          <ul>
            {instEvents &&
              instEvents.map((val) => {
                return <li>{val.installationType.name}</li>;
              })}
          </ul>
        </div>
